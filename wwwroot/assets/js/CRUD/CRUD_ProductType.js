function deleteSelectedProductTypes(ProductId) {
    var apiUrl = '/api/ProductTypeApi/deleteAll';
    ...
    .then(result => {
        ...
        if (result.message.toLowerCase().includes('successfully')) {
            ...
            alert('Failed to delete ProductType: ' + result.message);
            window.location.reload();
        }
    })
    .catch(error => {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('deleteSelected').addEventListener('click', function () {
        ...
        if (selectedProductIds.length > 0) {
            ...
            deleteSelectedProductTypes(selectedProductIds);
            window.location.reload();
        } else {
